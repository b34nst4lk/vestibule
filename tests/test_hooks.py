...
from vestibule.hooks import (
    ...
    vestibule_register_prompts,
    vestibule_register_resources,
    vestibule_register_tools,
    vestibule_validate_secrets,
)
from vestibule.plugin_manager import PluginManager
...
class TestHookSpecs:
    ...
    def test_vestibule_register_plugin_info_hook(self):
        """Test that vestibule_register_plugin_info hook spec exists."""
        assert vestibule_register_plugin_info is not None

    def test_vestibule_register_tools_hook(self):
        """Test that vestibule_register_tools hook spec exists."""
        assert vestibule_register_tools is not None

    def test_vestibule_register_resources_hook(self):
        """Test that vestibule_register_resources hook spec exists."""
        assert vestibule_register_resources is not None

    def test_vestibule_register_prompts_hook(self):
        """Test that vestibule_register_prompts hook spec exists."""
        assert vestibule_register_prompts is not None

    def test_vestibule_validate_secrets_hook(self):
        """Test that vestibule_validate_secrets hook spec exists."""
        assert vestibule_validate_secrets is not None
...
class MockPlugin:
    ...
    @hookimpl
    def vestibule_register_plugin_info(self):
        meta = PluginMetadata(
            name="mock-plugin",
            version="0.1.0",
    ...
    @hookimpl
    def vestibule_register_resources(self, mcp_server: FastMCP) -> None:
        @mcp_server.resource("mock://config")
        def get_config() -> dict:
            return {"key": "value"}

    @hookimpl
    def vestibule_register_prompts(self, mcp_server: FastMCP) -> None:
        @mcp_server.prompt()
...
class TestPluginManager:
    ...
    def test_get_metadata(self):
        ...
        result = pm.pm.hook.vestibule_register_plugin_info()
